...
# *** Computes Rossby Wave Source regressed against SST PC1 *** 
...
eta = np.mean(eta,axis=2)[:,:,np.newaxis] #Time-mean absolute vorticity
div = w.divergence()
div = np.ma.anom(div,axis=2) #Anomalous divergence
uchi, vchi = w.irrotationalcomponent() #Irrotational wind component
uchi = uchi-np.mean(uchi,axis=2)[:,:, np.newaxis]
vchi = vchi-np.mean(vchi,axis=2)[:,:, np.newaxis]

etax, etay = w.gradient(eta) #Vorticity gradient 
#S = -eta*div  - (uchi*etax + vchi*etay) #Rossby Wave Source (Total)
S = -(uchi*etax + vchi*etay) #Rossby Wave Source (vorticity advection)
...
#Adding cyclic point to map 
plot_map, plons_c = add_cyclic_point(r_map, coord=lon)
plons_c[-1] = 359.9999 

#Plots
levels = np.arange(-1e-10,1.1e-10,0.2e-10) #colorbar levels
...
#text(0.05, 0.9,"i)", ha='center', va='center',bbox=dict(facecolor='white', alpha=0.7), transform=ax.transAxes) #subfigure label 

plt.colorbar(shrink=0.7) 
plt.title(r"200 hPa DJF RWS ($-\vec{{v}}'_{{\chi}}\cdot\nabla\bar{{\zeta}}$ term) (s$^{{-1}}$) - {}".format(identifier)) 
plt.savefig("./figures/regression-maps-platecarree/SST_PC1+RWS_reg_{}.png".format(tag),bbox_inches='tight')
plt.show()
