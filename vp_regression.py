...
# *** Computes Velocity Potential regressed against SST PC1 *** 
...
#Plots
levels = np.arange(-4.8e6,4.9e6,8e5) #colorbar levels
...
#text(0.05, 0.9,"b)", ha='center', va='center',bbox=dict(facecolor='white', alpha=0.7), transform=ax.transAxes)

cbar = plt.colorbar(shrink=0.7) 
cbar.formatter.set_powerlimits((0,0))
cbar.update_ticks()
plt.title("200 hPa DJF Velocity Potential (m$^{{2}}$/s) - {}".format(identifier)) 
plt.savefig("./figures/regression-maps-platecarree/SST_PC1+chi_{}.png".format(tag),bbox_inches='tight')
plt.show()
