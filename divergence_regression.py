...
# *** Computes Diverge regressed against SST PC1 *** 
...
sf = recover_data(div, uwnd_info)
mean = np.mean(vp,axis=0)

tsDJF, dumyear = getseasonmean(ts,0)
tsDJF, lats, lons = getareabounds(tsDJF,-30,30,140,290) 
S_DJF, uyear = getseasonmean(sf,0)
S_DJF, lat, lon = getareabounds(S_DJF,-90,90,0,360)
...
plot_map, plons_c = add_cyclic_point(r_map, coord=lon) #r_map, corr_map_masked
plons_c[-1] = 359.9999 

#Plots
levels = np.arange(-3e-6,3.1e-6,0.5e-6) #colorbar levels

ax = plt.axes(projection=ccrs.PlateCarree(180)) 
plt.contourf(plons_c,-lat,plot_map,levels,transform=ccrs.PlateCarree(),cmap=plt.cm.RdBu_r,extend='both')
ax.coastlines()
#ax.set_extent([-179.99999, 179.999999, -30, 30], crs=ccrs.PlateCarree())
...
#text(0.05, 0.9,"c)", ha='center', va='center',bbox=dict(facecolor='white', alpha=0.7), transform=ax.transAxes) #subfigure label 

cbar = plt.colorbar(shrink=0.7) 
cbar.formatter.set_powerlimits((0,0))
cbar.update_ticks()
plt.title("200 hPa DJF divergence (s$^{{-1}}$) - {}".format(identifier)) 
plt.savefig("./figures/regression-maps-platecarree/SST_PC1+div_reg_{}.png".format(tag),bbox_inches='tight')
plt.show()
